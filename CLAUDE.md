...
- **`DELETE /admin/players/:id` no borra: es un soft delete.** Lo único que escribe es `deactivated_at` (y apaga `entry_match_available`), más la salida de la escalerilla compactando puestos y la liberación de sus canchas (`ReservationsService.cancelActiveForPlayer`, motivo `'Cancelada por baja del socio'` — no cuenta como cancelación tardía, y solo toca las reservas que todavía no empezaron). **No toca ni un dato** — nombre, email, teléfono, avatar, username, contraseña, récord y notificaciones quedan intactos — para que `POST /admin/players/:id/restore` devuelva la cuenta entera el día que el socio vuelve. Borrar de verdad se llevaría los desafíos del historial del RIVAL y descuadraría el fixture; además la FK lo bloqueaba y el endpoint devolvía 500.
