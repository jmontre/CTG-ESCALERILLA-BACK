...
- **`DELETE /admin/players/:id` no borra, da de baja**: si el jugador no tiene rastro (ni desafíos, ni partidos de Master, ni reservas, ni standings) se borra de verdad; si tiene historial se **desactiva** — sin datos de contacto, sin login, fuera de la escalerilla y con `deactivated_at` puesto, que es lo que lo saca de la lista pública y del panel. Borrarlo de verdad se llevaría los desafíos del historial del RIVAL y descuadraría el fixture. La respuesta trae `mode: 'deleted' | 'deactivated'`. **Al agregar un listado de jugadores nuevo, filtrar `deactivated_at: null`.**
- **El `name` de un jugador dado de baja NO se toca**, a propósito: es lo que hace que el fixture y el historial del rival sigan diciendo contra quién se jugó. Se probó con "Socio retirado" y dejaba los partidos ilegibles. Lo que se libera son `email` y `username` (con sufijo del id), para que la persona pueda volver a registrarse.
