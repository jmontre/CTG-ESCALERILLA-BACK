...
Implementado en `ChallengesService.reject` y `accept`, en las dos ramas de `processDoubleConfirmation` (resultado confirmado y disputa), en los tres flujos del cron (`handleNotAccepted`, `handleNotPlayed`, `handleNotConfirmed`) y en `AdminChallengesService.resolveChallenge` y `cancelOpenForPlayer`. **Toda transición que mueva posiciones o notifique va con claim**: sin él, dos requests que leyeron la fila antes de que cambiara el status corren el corrimiento, las stats y las notificaciones una vez cada una (pasó en producción: un resultado procesado 5 veces, con 5 tandas de WhatsApp). Tests: `challenges.result.spec.ts`.

`resolveChallenge` (admin) no lo tenía y en sep-2026 procesó **4 resultados dos veces en una semana**: la resolución tarda varios segundos, el admin volvía a apretar el botón y la segunda petición sumaba otra victoria y otra derrota. Hoy: claim sobre el estado leído antes de cualquier efecto, y si algo falla después se **suelta el claim** (vuelve al estado anterior) para poder reintentar. Sobre un desafío **ya completado** ("Actualizar Resultado" en el panel) no se vuelve a aplicar nada: con el mismo ganador solo corrige el marcador, y cambiar de ganador se rechaza (hay que anularlo y volver a resolverlo). Tests: `admin-resolve.spec.ts`. Para detectar estadísticas infladas, comparar `players.wins/losses/total_matches` con los desafíos `completed` de la temporada abierta; la huella de un procesamiento repetido son avisos `result_confirmed` duplicados del mismo partido.
